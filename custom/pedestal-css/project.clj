(defproject pedestal-css "0.0.1-SNAPSHOT"
  ...
  :aliases {"run-dev" ["trampoline" "run" "-m" "pedestal-css.server/run-dev"]}
  :repl-options  {:init-ns user
                  :init (try
                          (use 'io.pedestal.service-tools.dev)
                          (require 'pedestal-css.service)
                          ;; Nasty trick to get around being unable to reference non-clojure.core symbols in :init
                          (eval '(init pedestal-css.service/service #'pedestal-css.service/routes))
                          (catch Throwable t
  ...
  :main ^{:skip-aot true} pedestal-css.server)
