(defproject cors "0.0.1-SNAPSHOT"
  :description "Cross-origin resource sharing using Pedestal and ring middleware"
  ...
  :aliases {"run-dev" ["trampoline" "run" "-m" "cors.server/run-dev"]}
  :repl-options  {:init-ns user
                  :init (try
                          ...
                          (eval '(init cors.service/service #'cors.service/routes))
                          (catch Throwable t
