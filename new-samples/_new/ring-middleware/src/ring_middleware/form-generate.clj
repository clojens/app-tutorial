...
(defn attach-meta
  "Will associate the keys and values of metas with the head element of the page outline."
  [head metas]
  (assoc head (count head)
    (map #(vector :meta {:name (name %1) :content %2})
         (keys metas) (vals metas))))

'(
  (defn defpager [& {:keys [route title metas hooks style body+]}]
    body+)
    ...
    (defpager
      ...
      :metas {:keywords []
              :description ""}
      :hooks ["javascript:function(){include();};"]
      :style {:externalize true}
      :body+ '(
               [:article
                (styler {} ; ~> .foo{} (default) else :id ~> #bam{}
                [:section.foo {:name "baz" :id "bam"} "Bara"])
                 ]

               ))

  ;(attach-meta (head "fooo") {:keywords "bar, baz, bara" :description "bara baz lorem ipsum"})
)
